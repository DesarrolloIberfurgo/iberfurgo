...
function ppb_gallery_grid_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			...
			{
			    $return_html.= '<a href="'.esc_url($grandcarrental_purchase_url).'" title="'.esc_html__('Purchase', 'grandcarrental-custom-post' ).'" class="button tooltip"><i class="fa fa-shopping-cart"></i></a>';
			}
	    ...
	    if($all_photo_count > $current_photo_count && !empty($pagination))
	    {
			...
			$return_html.= '<a href="javascript:;" id="infinite_load_more_'.$custom_id.'" class="infinite_load_more" data-start="'.intval($current_photo_count).'" data-items="'.intval($tg_gallery_pagination).'">'.esc_html__('Load more', 'grandcarrental-custom-post' ).'</a>';
...
function ppb_car_search_func($atts, $content) {
	...
	$return_html.= '<div class="standard_wrapper">';
	$return_html.= '<div class="page_content_wrapper"><div class="inner">';
	$return_html.= '<form class="car_search_form ibf_background_black" method="get" action="busqueda">';
    $return_html.= '<div class="car_search_wrapper">';
    
    $return_html.= '<div class="one_fourth ibf_widthm_100 ibf_no_background ibf_mr_0">
						<p class="ibf_font_16 ibf_font_bold ibf_color_white ibf_field_form ibf_label_form">Oficina recogida/devolución</p>';
	...
	$return_html.= 			'<input id="fecha_inicio_dp" name="fecha_inicio" type="date" class="ibf_datepicker" value="'.esc_attr($atts['fechainicio']).'">';
	$return_html.=		'</div>';

	$return_html.= 		'<div class="one_third themeborder ibf_mr_0" style="width:30%;">';
	$return_html.= 			'<select name="hora_inicio" class="ibf_field_form ibf_width_100"><option value="">Hora</option>';
	$return_html.= 				$horas;
	$return_html.=			'</select>';
	$return_html.=		'</div>';
	$return_html.=	'</div>';
    ...
    $return_html.=	'<div class="one_fourth ibf_widthm_100 ibf_no_background">
						...
						<div class="one_half themeborder ibf_mr_0 ibf_datepicker_parent">';
	$return_html.= 			'<input id="fecha_fin_dp" name="fecha_fin" type="date" class="ibf_datepicker" value="'.esc_attr($atts['fechafin']).'">';
    $return_html.= 		'</div>';
	...
	$return_html.=	'<div class="one_sixth ibf_mr_0">
    					<input id="car_search_btn" type="submit" class="button ibf_btn_form" value="'.esc_html__( 'RESERVAR', 'grandcarrental-custom-post' ).'"/>
    				</div>
					</div>
				</form>';
...
function ppb_car_classic_func($atts, $content) {
			...
			if(!empty($image_url[0]))
			{
				...
				if(!empty($overall_rating))
				{
					...
					if($overall_rating_count > 0)
					{
						...
						{
							$return_html.= esc_html__('reviews', 'grandcarrental-custom-post' );
						}
						else
						{
							$return_html.= esc_html__('review', 'grandcarrental-custom-post' );
						}
				...
				$car_passengers = get_post_meta($portfolio_ID, 'car_passengers', true);
				...
				if(!empty($car_passengers) OR !empty($car_luggages) OR !empty($car_transmission))
				{
					...
					if(!empty($car_transmission))
					{
						...
						<div class="one_fourth">
							...
							<div class="car_attribute_content">'.grandcarrental_get_transmission_translation($car_transmission).'
							</div>
						</div>';
					}
	...
	{
		$return_html.= '<div class="car_search_noresult"><span class="ti-info-alt"></span>'.esc_html__("We haven't found any car that matches you're criteria", 'grandcarrental-custom-post').'</div>';
	}
		...
		foreach($posts_arr as $key => $ppb_post)
		{
			...
			$return_html.= '
				<div class="post_button_wrapper">
			    	<a class="readmore" href="'.esc_url(get_permalink($ppb_post->ID)).'">'.esc_html__('Read More', 'grandcarrental-custom-post' ).'<span class="ti-angle-right"></span></a>
					</div>
			</div>';
