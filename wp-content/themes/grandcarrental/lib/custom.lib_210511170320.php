...
function grandcarrental_get_instagram_response_without_token_from_json( $user ) {
...
function grandcarrental_get_instagram_response_without_token( $user ) {
...
{   
    ...
    if(!empty($username))
    {
		...
		$pp_is_update_instagram_grandcarrental = get_option('pp_is_update_instagram_grandcarrental');
		
		if(!file_exists($instagram_cache_path) OR $instagram_cache_timer > 300 OR empty($pp_is_update_instagram_grandcarrental))
		{
			$result = grandcarrental_get_instagram_response_without_token_from_json($username);
			...
			if(!empty($photos_arr))
			{
				...
				if(empty($pp_is_update_instagram_grandcarrental))
				{
					//updated Instagram cache to new API
					update_option('pp_is_update_instagram_grandcarrental', 1);
				}
			}
			else
			{
...
function grandcarrental_check_instagram_authorization() {
	...
	{
		$instagram_widget_settings_notice = 'Required plugin "Meks Easy Photo Feed Widget" is required. <a href="'.admin_url("themes.php?page=install-required-plugins").'">Please install the plugin here</a>.</a>';
	}
	else
	{
...
function grandcarrental_get_instagram_using_plugin($cache_prefix = 'photostream', $limit = 30) {
	...
	//delete_option('grandcarrental_'.$cache_prefix.$limit);
	
	//Get Instagram cached data
	$instagram_cache_data = get_option('grandcarrental_'.$meks_instagram_settings['user_id'].$limit);
	//delete_option('grandcarrental_'.$meks_instagram_settings['user_id'].$limit);
	...
	if(!isset($instagram_cache_data['data']) OR empty($instagram_cache_data['data']) OR $instagram_cache_timer > 30000)
	{
		...
		if(!empty($photos_arr))
		{
			...
			update_option('grandcarrental_'.$meks_instagram_settings['user_id'].$limit, $instagram_cache_data);
...
function grandcarrental_connect_fs()
{
  $wp_filesystem = grandcarrental_get_wp_filesystem();
...
function grandcarrental_hex_darker($rgb, $darker=2)
{
